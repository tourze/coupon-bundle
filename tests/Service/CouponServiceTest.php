...
namespace CouponBundle\Tests\Service;

use Carbon\Carbon;
use CouponBundle\Entity\Code;
use CouponBundle\Entity\Coupon;
use CouponBundle\Entity\Requirement;
use CouponBundle\Enum\RequirementType;
use CouponBundle\Event\CodeLockedEvent;
use CouponBundle\Event\CodeRedeemEvent;
use CouponBundle\Event\CodeUnlockEvent;
use CouponBundle\Event\CountCouponValidStockEvent;
use CouponBundle\Event\DetectCouponEvent;
use CouponBundle\Exception\CouponNotFoundException;
...
use CouponBundle\Repository\CouponStatRepository;
...
class CouponServiceTest extends TestCase
{
    ...
    public function testGetCouponValidStockFromRepository(): void
    {
        $coupon = $this->createMock(Coupon::class);
        
        $event = new CountCouponValidStockEvent();
        ...
        $result = $this->couponService->getCouponValidStock($coupon);
        
        $this->assertEquals(5, $result);
    }
    
    public function testGetCouponValidStockFromEvent(): void
    {
        $coupon = $this->createMock(Coupon::class);
        ...
        $this->eventDispatcher->expects($this->once())
            ->method('dispatch')
            ->willReturnCallback(function (CountCouponValidStockEvent $event) use ($coupon) {
                $this->assertEquals($coupon, $event->getCoupon());
                $event->setCount(10);
                return $event;
            });
        
        // 如果从事件中获取了计数，不应该调用仓库
        $this->codeRepository->expects($this->never())
            ->method('count');
